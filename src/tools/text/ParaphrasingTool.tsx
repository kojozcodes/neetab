...
export default function ParaphrasingTool() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-bold text-surface-600 dark:text-surface-400 mb-1.5">Style</label>
        <div className="flex flex-wrap gap-2">
          {STYLES.map(s => (
            <button key={s.value} onClick={() => setStyle(s.value)}
              className={`px-3 py-1.5 text-xs font-semibold rounded-md border transition-colors ${style === s.value ? 'bg-brand-500 text-white border-brand-500' : 'border-surface-300 dark:border-surface-600 text-surface-600 dark:text-surface-400 hover:border-brand-400'}`}>
              {s.label}
            </button>
          ))}
        </div>
      ...
      <div>
        <label className="block text-xs font-bold text-surface-600 dark:text-surface-400 mb-1.5">
          Original text <span className="font-normal text-surface-400">({input.length}/8000)</span>
        </label>
        <textarea className={`${taClass} h-36`} placeholder="Paste the text you want to paraphrase..." value={input} onChange={e => setInput(e.target.value)} maxLength={8000} />
      </div>
      ...
      {result && (
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-xs font-bold text-surface-600 dark:text-surface-400">Result</label>
            <button onClick={copy} className="text-xs text-surface-400 hover:text-brand-500 transition-colors">
              {copied ? '✓ Copied' : 'Copy'}
            </button>
          </div>
          <textarea readOnly className={`${taClass} h-36`} value={result} />
        </div>
      )}
    </div>
  );
