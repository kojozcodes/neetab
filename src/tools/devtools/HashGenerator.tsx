...
const ALGOS = [
  ...
  { id: 'SHA-1', label: 'SHA-1', len: 40 },
  { id: 'SHA-256', label: 'SHA-256', len: 64 },
  { id: 'SHA-512', label: 'SHA-512', len: 128 },
];
...
export default function HashGenerator() {
  ...
  const generate = useCallback(async (text: string) => {
    ...
    for (const algo of ['SHA-1', 'SHA-256', 'SHA-512']) {
      results[algo] = await hash(algo, text);
    }
    setHashes(results);
  }, []);
    ...
    <div>
      ...
      {Object.keys(hashes).length > 0 && (
        <div className="space-y-2">
          {ALGOS.map(a => hashes[a.id] && (
            <ResultBox key={a.id} label={a.label} value={hashes[a.id]} large={false} />
          ))}
        </div>
      )}
      ...
      {!input && (
        <div className="text-center py-4 text-surface-400 text-xs">
          Type text above to generate hashes instantly
        </div>
      )}
    </div>
  );
